...
-- 정보 갱신 시각 // UPDATE
-- 업데이트 시간
-- 2번 유저가 로그인을 한 다음 쿼리문이 실행된다고 가정함
-- -----------------------------------------------------
UPDATE user SET userUpdaAt = CURRENT_TIMESTAMP WHERE userNo = 2;
-- -----------------------------------------------------
-- 경매 등록 // INSERT
-- 경매가격, 경매 종료시간, 경매 이미지, 경매 상세설명, 경매를 올린 유저의 아이디(userNo)
...
-- -----------------------------------------------------
-- 책 정보 등록 // INSERT
-- 경매 아이디, 책 제목, 저자, 출판사, 출판일, ISBN, 책 설명
...
-- -----------------------------------------------------
-- 입찰 등록 // INSERT
-- 입찰 가격, 입찰 된 물건의 이미지, 입찰 된 물건의 상세 설명, 경매 아이디, 입찰자의 유저 아이디(userNo)
-- bidId -> 숫자 자동 증가 (AI)
-- 이미지 주소는 img/bid라는 폴더를 고정으로 둔다고 가정
-- aId는 외래키이므로 반드시 auction.auctionId 컬럼에 값이 있는걸 넣어야함
-- uId는 외래키이므로 반드시 user.userNo 컬럼에 값이 있는걸 넣어야함
-- -----------------------------------------------------
select * from bid;
INSERT INTO 
	bid 
